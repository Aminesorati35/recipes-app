  ...
  useEffect(() => {
    const getRecipe = async () => {
      try {
        ...
        const MyIngredients = response.data.recipe.ingredients.map((el) => {
          return { name: el.name, quantity: el.quantity };
        });
        setIngredients(MyIngredients);
        const MyInformations = response.data.recipe.informations.map((el) => {
          return { name: el.name, quantity: el.quantity };
        });
        setInformations(MyInformations)
        setSteps(response.data.recipe.steps);
      } catch (e) {
        console.error(e);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response =  await authService.authenticatedRequest(
        `http://localhost:8000/api/recipes/update/${id}`,
       {
          method: "POST",
          body: formData,
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="text-center pt-6">
            ...
            >
              <FontAwesomeIcon icon={faPlus} />
              {isSubmitting ? "Modify..." : "Modify Recipe"}
            </button>
